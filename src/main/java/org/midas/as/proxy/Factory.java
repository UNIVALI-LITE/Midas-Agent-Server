...
/**
 * Esta classe atua como um manipulador de eventos e um Factory Method ataves do
 * qual e possivel abstrair o processo de criacao de instancias de organiza-
...
public class Factory {
	...
	public void invokeEntity(String organizationName, String serviceName, Map in, List out)
			throws FactoryException, ServiceException {
		try {
			String path = Catalog.getServicePath(organizationName, serviceName);
			ServiceInfo sinfo = Catalog.getServiceByName(organizationName, serviceName);
			Provider provider = instantiateEntity(path, sinfo.getPort());
			...
			try {
				...
				throw new FactoryException("Unable to find " + entityName
						+ " entity class, verify if declaration on structure.xml points to a valid class", e);
			} catch (CatalogException e1) {
				throw new FactoryException(
						"Severe error on the consistence of information about the structure, recommended to check structure.xml and re-start the server",
						e1);
			}
	...
	public Provider instantiateEntity(String entityClassName, Object port)
			...
			InvocationTargetException, NoSuchMethodException, SecurityException
	{
		...
		((Agent)entityObject).setPort((String) port);
